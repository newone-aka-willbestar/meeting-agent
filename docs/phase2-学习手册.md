...
> [mcp-server/server.py](../mcp-server/server.py)（FastMCP，stdio）+ [README](../mcp-server/README.md)（接 Claude Desktop）。
> 后端配套：`Todo` 表 + [api/todos.py](../backend/app/api/todos.py)（`POST/GET /todos`）；worker 抽取后自动落待办（source=agent）。

MCP（Model Context Protocol）是个**标准协议**，把能力暴露成 tools，
任何兼容的 client（你的 agent、Claude Desktop）都能发现并调用。

**已实现的 3 个工具**（用官方 MCP Python SDK 的 FastMCP）：
- `create_todo` → `POST /todos`
- `search_meeting_history` → `GET /search`
- `get_minutes` → `GET /meetings/{id}/extraction`

**关键设计**：
- MCP server 是**协议适配器**，自己不碰数据库，全部转调后端 REST——职责单一、好讲。
- 传输用 **stdio**：Claude Desktop 把脚本当子进程拉起、用标准输入输出通信。
- httpx 设 `trust_env=False` 绕过本机代理对 localhost 的劫持（踩过的坑）。

**已验证**：建独立 venv 装 mcp+httpx，直接调三个工具打活后端，全部成功
（`create_todo` 真建了待办、`search` 跨会命中、`get_minutes` 取回纪要）。接 Claude Desktop 见 README。

🔍 问 AI：
- MCP 的 server/client 怎么握手、agent 怎么发现工具（tool discovery）？
- 为什么同一个 server 能被 Claude Desktop 这种通用 client 连上？
- stdio 传输 vs SSE/HTTP 传输，分别什么场景？
- "协议适配器"这种把已有 REST 包成 MCP 的做法，有什么利弊？
